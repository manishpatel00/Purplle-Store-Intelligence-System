...
### Summary
- **AI was right on:** Async architecture, PostgreSQL for production, FastAPI framework, modular pipeline design, comprehensive testing
- **AI was wrong on:** Detection model choice (suggested m/m+), session-first schema, overly defensive error handling
- **Where I trusted my judgment:** Edge case handling (re-entry logic), real-time dashboard streaming first, practical constraints (15fps frame rate binding)

### Key AI Contributions
1. **Claude (Anthropic):** Suggested async database design, PostgreSQL over SQLite, importance of idempotency in event ingest
2. **GPT-4 (OpenAI):** Suggested RT-DETR for occlusion handling, WebSocket architecture for real-time updates
3. **Both:** Emphasized production-readiness, comprehensive testing (86% coverage achieved), structured logging
...
See [docs/CHOICES.md](docs/CHOICES.md) section 4 for full reasoning. Summary: `conversion_rate_pct` uses POS transactions correlated to `BILLING_QUEUE_JOIN` events in the 5-minute pre-transaction window (`app/pos.py`), mapping `ST1008` → `STORE_BLR_002`.

---

## 5. Dashboard API Routing: Nginx Same-Origin Proxy

See [docs/CHOICES.md](docs/CHOICES.md) section 5. The dashboard Docker image proxies `/api`, `/health`, and `/ws` to the API service so reviewers need only open `http://localhost:5173`.
